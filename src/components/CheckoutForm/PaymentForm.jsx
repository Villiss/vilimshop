...
const PaymentForm = ({ checkoutToken, shippingData, backStep, onCaptureCheckout, nextStep, timeout }) => {
    ...
    return (
        <>
            ...
            <Divider />
            <Typography variant="h6" gutterBottom style={{ margin: '20px 0' }}>Spôsob platby</Typography>
                    ...
                    {({ elements, stripe }) => (
                        <form onSubmit={(e) => handleSubmit(e, elements, stripe)}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Button variant="outlined" onClick={backStep}>Späť</Button>
                                <Button type="submit" variant="contained" disabled={!stripe} color="primary">
                                    Zaplatiť { checkoutToken.live.subtotal.formatted_with_symbol }
                                </Button>
                            </div>
                        </form>
                    )}
